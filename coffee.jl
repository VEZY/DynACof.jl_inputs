using DynACof
(
Stocking_Coffee   = 5580.0,     # Coffee density at planting (plant ha-1)
...
WoodPruningRate   = 1.0/3.0,    # how much branches wood are pruned (ratio)
...
F_buds2           = 2562.0,     # Bud development stage 2 (dd)
...
F_Tffb            = 4000.0,     # Time of first floral buds (Rodriguez et al. 2001).
...
F_rain            = 40.0,       # Amount of cumulative rainfall to break bud dormancy (mm). Source: 20 mm Zacharias et al. (2008)
...
BudInitEnd        = 100.0,      # End of bud initiation period relative to first potential bud break of the year (dd).
# FruitMaturation   = 2836.0     # Fruit maturation duration until stage 5 ripe (dd). Source: Rodriguez et al. (2011) Table 1.
F_over            = 3304.0,     # Duration until fruit stage 5 overripe in  the  soil (dd). Source: Rodriguez 2011 Table 1
u_log             = 1418.0,     # Parameters for the logistic fruit growth pattern (FruitMaturation/2)
s_log             = 300.0,      # Idem
...
Min_Fruit_CM      = 20.0,       # Minimum fruit carbon mass below which harvest cannot be triggered
...
lifespan_Leaf     = 265.0,      # Leaf life span. Source: Charbonnier et al. (2017)
lifespan_Shoot    = 7300.0,     # Resprout wood life span. Source: Van Oijen et al (2010 I)
lifespan_SCR      = 7300.0,     # Stump and coarse roots life span. Source: Charbonnier et al. (2017)
lifespan_FRoot    = 365.0,      # Fine roots life span. Source: Van Oijen et al (2010 I)
...
SlopeAzimut       = 180.0,      # site slope azimuth (deg)
Slope             = 5.0,        # Percentage slope (%)
